  ...
  return (
    <>
      <div className="w-full min-h-full bg-white dark:bg-[#333334] p-5 rounded-b-lg relative">
        <div className="flex gap-3 mb-1 bg-[#d0edc191] dark:bg-[#3D3D3E] backdrop-blur-[20px] rounded-xl p-2 border border-white/10">
          <button
            className={`flex-1 py-3 px-2 md:px-5  rounded-lg cursor-pointer  flex justify-center items-center ${
              activeTab === "active"
                ? " bg-[#73CC61] dark:bg-[#27272A] font-semibold text-white"
                : "text-white-50 "
            }`}
            onClick={() => setActiveTab("active")}
          >
            Accounts
            <span className="ml-2 inline-flex items-center justify-center min-w-[22px] h-[22px] bg-[#D0EDC1] dark:bg-[#828284] text-black dark:text-white border border-white text-xs font-medium rounded-lg px-2 shadow-md">
              {activeCount}
            </span>
          </button>
        ...
        {totalPages > 1 && (
          <div className="flex justify-center items-center gap-2 mt-8 flex-wrap">
            <button
              ...
              className="px-3 py-1 rounded-lg  bg-[#DAEBD3] disabled:opacity-50 cursor-pointer"
            >
              <FaAnglesLeft color="#70CB68" />
            </button>
            ...
            {Array.from({ length: totalPages }, (_, i) => (
              <button
                ...
                className={`px-3 py-1 rounded-lg  ${
                  currentPage === i + 1
                    ? "bg-[#70CB68] text-white dark:bg-[#27272A] cursor-pointer "
                    : "bg-[#DAEBD3]  dark:bg-[#3D3D3E]  cursor-pointer"
                }`}
              >
                {i + 1}
              </button>
        ...
        <button
          ...
          className="fixed bottom-30 right-10 md:bottom-6  md:right-6 bg-[#70CB68] dark:bg-[#27272A] text-white  md:w-15 md:h-15 w-13 h-13 rounded-full text-2xl shadow-lg flex items-center justify-center cursor-pointer hover:scale-110 transition-all duration-200"
        >
          <IoMdAdd />
        </button>
      </div>
